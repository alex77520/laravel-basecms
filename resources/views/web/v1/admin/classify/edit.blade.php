...
<form action="{{ route('/admin/posts/classify/edit') }}" method="post" id="edit-form">
{{ csrf_field() }}
    <div class="modal-body">
            @if ($classify->father != 0)
                <input type="hidden" value='{{ $classify->parent->pc_id }}' name="father">
                <div class="form-group">
                    <input name="name" type="text" class="form-control" placeholder="分类名称" disabled="disabled" value="{{ $classify->parent->name }}">
                </div>
            @endif
            ...
            <div class="form-group">
                <div>
                    ...
                    <label class="radio-inline">
                        <input type="radio" name="show" id="inlineRadio1" value="1" @if($classify->show == '1') checked @endif > 启用
                    </label>
                    <label class="radio-inline">
                        <input type="radio" name="show" id="inlineRadio2" value="2" @if($classify->show != '1') checked @endif  > 禁用
                    </label>
                </div>
            </div>
    </div>
