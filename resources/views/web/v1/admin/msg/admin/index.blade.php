		...
		<div class="row">
				...
				<div class="app-action-panel" id="inbox-action-panel">
					...
					<div class="app-actions-list scrollable-container ps-container ps-theme-default" data-ps-id="ba4c682c-c426-d139-2fde-3d9a24ab539f">
						<div class="list-group">
							<a href="{{route('/admin/notice/create')}}" class="text-color list-group-item"><i class="m-r-sm mdi mdi-note-plus"></i>新建站内信</a>
						</div><!-- .list-group -->
						...
						<div class="list-group">
                            ...
                            <form action="{{route('/admin/notice')}}" method="get">
                                <div class="form-group">
                                    <label for="uid">发送者</label>
                                    <select name="uid" id="uid" class="form-control" data-plugin="select2">
                                        <option value="0">不限</option>
                                        @foreach ($users as $user)
                                            @if (isset($search['uid']))
                                                @if ($user->uid == $search['uid'])
                                                    <option value="{{$user->uid}}" selected="selected">{{$user->nickname}}[{{$user->account}}]</option>
                                                @else
                                                    <option value="{{$user->uid}}">{{$user->nickname}}[{{$user->account}}]</option>
                                                @endif
                                            @else
                                                <option value="{{$user->uid}}">{{$user->nickname}}[{{$user->account}}]</option>
                                            @endif
                                ...
                                <div class="form-group">
                                    <label for="to">类型</label>
                                    <select name="to" id="to" class="form-control" data-plugin="select2">
                                        <option value="0" @if(isset($search['to'])) @if($search['to'] == '0') selected="selected" @endif @endif>不限</option>
                                        <option value="1" @if(isset($search['to'])) @if($search['to'] == '1') selected="selected" @endif @endif>站内信</option>
                                        <option value="2" @if(isset($search['to'])) @if($search['to'] == '2') selected="selected" @endif @endif>组内信</option>
                                    </select>
                                </div>
                                <div class="form-group">
			...
			<div class="col-md-10">
				<div class="row">
					<div class="col-md-12">
						<div class="m-b-lg">
							@if ($controller->HasLimit('MsgDels') !== false)
                        ...
                        <div class="widget p-lg">
                            ...
                            <table class="table table-hover">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if($notice->to == '1')
                                            <span class="text-success">站内信</span>
                                        @else 
                                            <span class="text-primary">组内信</span>
                                        @endif
                                    </td>
                                    <td>
                                       {{$notice->created_at}}
                                    ...
                                    <td>
                                        <div class="btn-group dropup">
                                            ...
                                            <ul class="dropdown-menu">
                                                <li><a href="{{ route('/admin/notice/intro',['id'=>$notice->notice_id]) }}" data-toggle="modal" data-target="#commonModal-lg">详细信息</a></li>
                                                <li><a href="{{ route('/admin/notice/intro/users',['id'=>$notice->notice_id]) }}" data-toggle="modal" data-target="#commonModal">阅读详情</a></li>
                                                @if ($controller->HasLimit('MsgDel') !== false)
                                                <li>
...
function doDel(id){
    $.confirm({
        ...
        confirm:function(){
            ...
            $.post("{{ route('/admin/notice/delete')}}",{notice_id:id},function(data){
                if(data.code == '1'){
...
function del(){
    ...
    $.confirm({
        ...
        confirm:function(){
            ...
            $.post("{{ route('/admin/notice/deletes')}}",{ids:ids},function(data){
                if(data.code == '1'){
                    Toast('成功删除 '+data.data+' 条记录');
                    setTimeout(function(){
                        window.location.reload();
