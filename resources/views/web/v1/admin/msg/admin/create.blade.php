				...
				<div class="app-action-panel" id="inbox-action-panel">
					...
					<div class="app-actions-list scrollable-container ps-container ps-theme-default" data-ps-id="ba4c682c-c426-d139-2fde-3d9a24ab539f">
                        <div class="list-group">
							<a href="{{route('/admin/notice/create')}}" class="text-color list-group-item"><i class="m-r-sm mdi mdi-account-multiple"></i>整站群发</a>
						</div><!-- .list-group --> 
                        <div class="list-group">
							<a href="{{route('/admin/notice/create',['type'=>'group'])}}" class="text-color list-group-item"><i class="m-r-sm mdi mdi-sitemap"></i>面向服务号</a>
						</div><!-- .list-group -->
                        <div class="list-group">
							<a href="{{route('/admin/notice/create',['type'=>'user'])}}" class="text-color list-group-item"><i class="m-r-sm mdi mdi-account"></i>面向用户</a>
						</div><!-- .list-group -->
						<!-- mail label list -->
                    ...
                    <div class="col-md-12">
                        <div class="widget p-lg">
                            <h4 class="m-b-lg">
                                ...
                                @if(isset($type))
                                    @if($type == 'group')
                                        <form action="{{ route('/admin/notice/create/group')}}" method="post" id="edit-form">
                                        <div class="alert alert-info">最多选择10个服务号进行通知。</div>
                                        <div class="form-group">
                                            <label for="group">通知范围：</label>
                                            <select name="group[]" id="group" class="form-control" data-plugin="select2" multiple data-options="{ minimumSelectionLength:1,maximumSelectionLength: 10 }">
                                    ...
                                    @else
                                        <form action="{{ route('/admin/notice/create/user')}}" method="post" id="edit-form">
                                        <div class="alert alert-info">最多选择50个用户进行通知。</div>
                                        <div class="form-group">
                                            <label for="group">通知范围：</label>
                                            <select name="user[]" id="group" class="form-control" data-plugin="select2"  multiple data-options="{minimumSelectionLength:1,maximumSelectionLength: 50 }" >
                                                @foreach ($groups as $group)
                                                    <optgroup label="{{$group->name}}">
                                                        ...
                                                        @if(count($group->GroupUsers) > 0)
                                                            @foreach($group->GroupUsers as $User)
                                                                <option value="{{$User->User->uid}}">{{$User->User->nickname}}({{$User->User->account}})</option>
                                                            @endforeach
                                                        @endif
                                                    </optgroup>
                                                @endforeach
                                    ...
                                    @endif
                                @else
                                    <form action="{{ route('/admin/notice/create')}}" method="post" id="edit-form">
                                @endif
                                {{ csrf_field() }}
                                <div class="form-group">
                ...
                success: function (data) {
                    if(data.code == '1'){
                        $.alert({
                            ...
                            confirm:function(){
                                window.location.href="{{route('/admin/notice')}}";
                            }
                        });
                    }else{
                        $.alert(data.data);
