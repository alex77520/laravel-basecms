...
<section class="app-content">
    ...
    <div class="row">
        <div class="col-md-12">
            <form action="{{ route('/admin/article/create/image-text') }}" name="" method="post" id="publish-form">
                {{ csrf_field() }}
                <div class="row">
                        ...
                        <div class="form-group">
                            ...
                            {{-- @if ($classifys != null)
                                @foreach ($classifys as $classify)
                                    ...
                                    <option value="{{ $classify->pc_id }}" disabled="disabled">{{ $classify->name }}</option>
                                    @else
        ...
        $('#publish-form').bootstrapValidator({
            ...
            $.ajax({
                ...
                success: function (data) {
                    if(data.code == '1'){
                        Toast('创建成功，跳转至添加条目页面');
                        setTimeout(function(){
                            window.location.href="{{ route('/admin/article/image-text/item')}}"+"?post_id="+data.data.id;
                        },1000);
                    }else{
                        $.alert(data.data);
                    }
